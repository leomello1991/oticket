...
class CreateSessionIdPagSeguroService {
  async execute({
    ...
    for (let index=0;index<itens.length;index++){
      infoDebit[`itemId${index+1}`] = (index+1).toString().padStart(4, '0');
      infoDebit[`itemQuantity${index+1}`] = itens[index].itemQuantity;
      infoDebit[`itemDescription${index+1}`] = itens[index].itemDescription;
      infoDebit[`itemAmount${index+1}`] = itens[index].itemAmount;
    }
    const headers = {
      'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
    };
    const parsedBodyContentTypeUrlencodedWwwX = qs.stringify(infoDebit)
    try {
      ...
      const {
        transaction:{
          date:{_text:date},
          code:{_text:code},
          reference:{_text:debit_reference},
          type:{_text:type},
          status:{_text:status},
          ...
          extraAmount:{
            _text:debit_extraAmount
          },
          installmentCount:{
            _text:installmentCount
          },
          itemCount:{
            _text:debit_itemCount
          },
          items:{item:items},
          sender:{
            name:{
      ...
      const debitInfoResponseDataChange = {
          date,
          code,
          ...
          feeAmount,
          netAmount,
          extraAmount:debit_extraAmount,
          installmentCount,
          itemCount:debit_itemCount,
          item:newItems,
          sender:{
            name:sender_name,
            email:sender_email,
      ...
      return {debitInfoResponseDataChange}
    }catch(error){
